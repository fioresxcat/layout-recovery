...
from doc_converter.converters import Converters
...
class Reconstructor:
    ...
    def predict(self, result):
        ...
        save_dir = os.path.join(self.model_cfg['save_folder'], result['request_id'])
        os.makedirs(save_dir, exist_ok=True)
        # visualize result
        for idx, (img, boxes, class_names, scores) in enumerate(zip(imgs, result['layout']['boxes'], result['layout']['class_names'], result['layout']['scores'])):
        ...
        converters = self.converters(result)
        converters.reconstruct(docx_path=f'docx_output/{result["request_id"]}.docx')
