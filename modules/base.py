...
class BaseModule(object):
    ...
    def add_metadata(self, metadata, num_request, batch_size):
        if self.__class__.__name__ not in metadata:
            metadata[self.__class__.__name__] = {
                'num_request': 0,
                'total_batch_size': 0
            }
    ...
    def request(self, data):
        ...
        while num_request < self.max_request_time:
            try:
                num_request += 1
                results = self.triton_client.infer(model_name=self.model_config['model_spec_name'], inputs=infer_inputs, outputs=infer_outputs, client_timeout=int(self.model_config['request_timeout']))
                break
            except:
