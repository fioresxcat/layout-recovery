...
class BaseYOLODetector(BaseModule):
    instance = None
    
    def __init__(self, common_config, model_config):
        super(BaseYOLODetector, self).__init__(common_config, model_config)
    ...
    def get_instance(common_config, model_config):
        if BaseYOLODetector.instance is None:
            BaseYOLODetector.instance = BaseYOLODetector(common_config, model_config)
        return BaseYOLODetector.instance
    ...
    def is_poly_belong(self, text, block):
        ...
        x3, y3 = text[2]
        x4, y4 = text[3]
        xmin_text = min(x1, x2, x3, x4)
        xmax_text = max(x1, x2, x3, x4)
        ymin_text = min(y1, y2, y3, y4)
        ymax_text = max(y1, y2, y3, y4)
